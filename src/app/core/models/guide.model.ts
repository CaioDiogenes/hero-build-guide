...
export type ControlRestriction = 'Basic' | 'Ultimate' | 'Basic and Ultimate' | 'Other';
...
export interface DotInteraction {
  type: 'Increase damage' | 'Immunity' | 'Remove effect';
  source: string;
  description: string;
}
...
export interface GuideAcronym {
  ...
  meaning: string;
  description: string;
  category: 'Role' | 'Combat' | 'Game mode' | 'Attack';
}
